<template>
  <div id="app">
    <right-panel @change="panelChange">
      ...
      <table-code v-else :api-config="apiConfig" :api-table-list="apiTableList"></table-code>
    </right-panel>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getFieldListByDocs(apiJson, operationId) {
      ...
      // 1. 获取api相关信息内容
      this.apiConfig.basePath = apiJson.basePath
      Object.keys(paths).some((item) => {
        this.apiConfig.url = item
        ...
        let flag = Object.keys(methods).some((methodsItem) => {
          ...
          if (methodsConfig.operationId === operationId) {
            ...
            this.apiConfig.method = methodsConfig.methods
            return true
          }
        })
        return flag
